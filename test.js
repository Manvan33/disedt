...
client.on('message', function(msg){
	...
	if (msg.content.startsWith('!')) { // Détecte une requête du bot (!)
		var reponse=[];
		var command = msg.content.slice(1,msg.content.length).split(" "); //Découpe la commande en mots
		var args = command.slice(1,command.length); //Stocke les arguments
		...
		if (command=='edt') {// Assigne initialement la variable TP en fction du role
			...
			if (msg.member.roles.has(msg.guild.roles.find("name", "TP1").id)) {
				var TP = "TP1";
			}
				...
				for (i in args) { //Trouve un potentiel argument "tp." avec . le n° du TP
					...
					if (args[i].match(/^([0]?[0-9]|[12][0-9]|[3][01])\/[0]?[1-9]|[1][012]/)) {
						...
						if (parseInt(args[i].slice(args[i].indexOf("\/")+1)) < 13) {
							...
							dateD = args[i].slice(0,args[i].indexOf("\/"));
							dateM = args[i].slice(args[i].indexOf("\/")+1);
						}
					}
				}
			}
			...
			if ((events.length<1)||(parseInt(events[events.length-1].DTEND.slice(8,12)) < (parseInt(actual)-2)&&date==today())) {
				if (events.length>0) {
					events=[];
				}
